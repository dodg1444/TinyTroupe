...
def setup_logging(logger_name: str = "tinytroupe", log_level: str = "INFO", api_type: str = "azure"):
    class ColoredFormatter(logging.Formatter):
        ...
        def format(self, record):
            ...
            return (
                f"{self.COLORS.get(record.levelname, '')}{log_message}{Style.RESET_ALL}"
            )
    ...
    log_filename = f"{logger_name}_{timestamp}_{api_type}.log"
...
def start_logger(config: configparser.ConfigParser):
    ...
    setup_logging("tinytroupe", log_level, config["OpenAI"]["API_TYPE"])
...
class JsonSerializableRegistry:
    ...
    @classmethod
    def from_json(
        cls, json_dict_or_path, suppress: list = None, post_init_params: dict = None
    ):
        """
